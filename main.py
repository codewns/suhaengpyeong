...
추천 1: (구체적인 한국어 주제명)
0. 선정 근거: (수행평가 안내문을 꼼꼼히 분석한 뒤, 안내문에 여러 질문/항목/자료 목록 중 선택하는 구조가 있으면 반드시 "안내문의 N번 질문 '~(질문 내용 요약)'을 선택해 이 주제를 선정했습니다." 형식으로 작성. 자료 목록에서 고르는 수행이면 "안내문에서 제시된 자료 중 '~'를 선택해 이 주제를 선정했습니다." 형식으로 작성. 선택 구조가 전혀 없는 수행이면 이 항목 생략)
