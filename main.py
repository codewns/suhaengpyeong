...
0. 선정 근거: 수행평가 안내문을 꼼꼼히 분석한다.
   - 안내문에 학생이 반드시 하나를 골라야 하는 질문 목록, 주제 목록, 자료 목록이 있으면 → 반드시 "안내문에 제시된 N가지 질문 중 N번 '~(질문 내용 그대로 인용)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
   - 자료 목록에서 고르는 수행이면 → "안내문에서 제시된 자료 중 '~(자료명)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
   - 평가 기준(채점 기준, 점수 배분 등)은 선정 근거가 아니므로 절대 사용하지 않는다
   - 선택 구조(질문 목록/자료 목록)가 전혀 없는 수행이면 이 항목 전체 생략
