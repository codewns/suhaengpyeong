...
5. 항목은 숫자 번호(0. 1. 2. 3.)로 구분한다.
6. 말투는 자연스러운 한국어 문장으로 쓴다.
7. 추천 3개가 끝나면 추가 안내, 조언, 표절 방지 안내 등 어떤 내용도 절대 출력하지 않는다. 추천 3으로 끝낸다.

반드시 아래 형식으로 3개 추천:

추천 1: (구체적인 한국어 주제명)
0. 선정 근거: (안내문에 번호 매겨진 질문 목록 또는 자료 목록이 실제로 존재할 때만 작성. 없으면 이 항목 통째로 생략. AI가 질문을 만들어내는 것은 절대 금지.)
...
[선정 근거 작성 전 반드시 수행할 판단 절차]

STEP 1. 안내문을 읽고 아래 질문에 답하라:
  - 안내문에 "1번. ~~", "2번. ~~" 같은 번호가 매겨진 질문 목록이 실제로 존재하는가? YES / NO
  - 안내문에 학생이 골라야 할 도서/영상/자료 목록이 실제로 존재하는가? YES / NO

STEP 2. 판단 결과에 따라:
  - 번호 매겨진 질문 목록이 YES이면 → 그 질문을 직접 인용하여 "안내문에 제시된 N가지 질문 중 N번 질문 '(질문 내용 그대로)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
  - 자료/도서/영상 목록이 YES이면 → "안내문에서 제시된 자료 중 '(자료명)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
  - 둘 다 NO이면 → 0번 선정 근거 항목을 완전히 생략한다. 어떤 질문도 만들어내지 않는다.

절대 금지:
- 안내문에 없는 질문을 AI가 만들어서 인용하는 것
- "랜덤으로 뽑기", "질문 List에서 선택" 같은 수행 방식 설명을 선정 근거로 쓰는 것
- 평가 기준, 채점 기준, 점수 배분을 선정 근거로 쓰는 것

위 절차를 반드시 먼저 수행한 뒤, 핵심 원칙을 최우선으로 지키면서 맞춤 주제 3개를 추천해주세요.
...
- 링크는 반드시 실제 검색 결과에서 확인된 URL만 사용한다. 존재하지 않는 링크를 만들어내는 것은 절대 금지
- 링크가 확인되지 않으면 링크 항목을 생략한다
