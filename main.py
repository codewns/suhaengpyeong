...
추천 1: [구체적인 주제명]
- 핵심 내용: (2-3문장)
- 이전 주제와의 연결: (이전 주제가 있다면 어떻게 심화·확장되는지, 없으면 생략)
- 추후 심화 서사: "~에 흥미를 느껴 ~을 탐구했다. 이 주제는 ~와 연결되며, 이후 ~한 활동으로 심화할 계획이다." 형식으로 작성
- 추천 이유: (평가기준 연계 + 진로 연계 + 학년 특성 반영)
- 점수 강점: (어떤 평가 항목에서 높은 점수를 받을 수 있는지)
...
핵심 원칙 준수 사항:
- 불확실한 자료는 절대 추천하지 마세요 (원칙 6).
- 각 자료마다 평가 기준의 어떤 항목과 연결되는지 명시하세요 (원칙 4).
- 자료는 "참고용"임을 명시하고, 직접 인용이 아닌 학생 자신의 언어로 활용하도록 강조하세요 (원칙 3).
- 이 자료를 활용한 추후 심화 탐구 방향을 제시하세요 (원칙 2).
...
- 유형: (도서/영상/논문/사이트)
- 핵심 내용: (2-3문장)
- 수행평가 활용법: (구체적으로, 어떤 평가 항목에 연결되는지 포함)
- 추후 심화 연결: (이 자료를 발판으로 어떤 심화 탐구가 가능한지)
- 진로 연계:
...
추후 심화 탐구로 이어질 수 있는 자료를 우선 추천해주세요 (원칙 2).
"""
...
평가 시 반드시 확인할 사항:
- 평가 기준의 모든 항목이 포함되었는지 체크 (원칙 4) — 빠진 항목은 반드시 지적
- 표절이 의심되는 문장이 있으면 경고 (원칙 3)
- 이전 주제와의 심화·연결이 드러나는지 확인 (원칙 1)
- 추후 심화 탐구로 이어질 수 있는 마무리가 되어 있는지 확인 (원칙 2)
- 불확실한 사실이 포함되어 있으면 지적 (원칙 6)
- 학년({session.grade or '미입력'})에 맞는 탐구 수준인지 확인 (원칙 5)
...
추가 체크:
## 🔍 표절 위험 문장
(의심 문장이 있으면 인용, 없으면 "특이사항 없음")

## 🔗 심화 탐구 연결성
(이전 주제와의 연결, 추후 탐구 가능성이 잘 드러나는지 평가)
