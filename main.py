...
추천 1: (구체적인 한국어 주제명)
0. 선정 근거: 수행평가 안내문을 꼼꼼히 분석한다.
   - 안내문에 학생이 직접 골라야 할 질문 목록(예: 1번. ~~ / 2번. ~~ / 3번. ~~)이 있으면 → "안내문에 제시된 N가지 질문 중 N번 질문 '(질문 내용을 그대로 인용)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
   - 자료/도서/영상 목록에서 고르는 수행이면 → "안내문에서 제시된 자료 중 '(자료명)'을 선택해 이 주제를 선정했습니다." 형식으로 작성
   - 수행 방식 설명(예: "랜덤으로 뽑기", "질문 List에서 선택", "무작위 출제" 등)은 선정 근거가 절대 아니다
   - 평가 기준, 채점 기준, 점수 배분은 선정 근거가 절대 아니다
   - 실제 질문/자료 목록이 안내문에 명시되어 있어야만 작성한다. 없으면 이 항목 전체 생략
...
절대 금지: "랜덤으로 뽑기", "질문 List에서 선택" 같은 수행 방식 설명을 선정 근거로 쓰는 것
절대 금지: 평가 기준, 채점 기준, 점수 배분을 선정 근거로 쓰는 것
